...
export default function CommunityPage() {
  ...
  const handleAcceptFriend = async (requestId) => {
    try {
      // Optimistic update
      queryClient.setQueryData(QUERY_KEYS.friendships, (old) => {
        if (!old) return old;
        return old.map(f => f.id === requestId ? { ...f, status: 'accepted' } : f);
      });
      
      await base44.entities.Friendship.update(requestId, { status: 'accepted' });
      
      // Invalidate in background without triggering loading state if possible
      queryClient.invalidateQueries({ queryKey: QUERY_KEYS.friendships });
