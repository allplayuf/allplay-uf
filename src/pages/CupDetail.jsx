...
// Yellow theme status config
const STATUS_CONFIG = {
  upcoming: { label: 'Kommande', color: 'bg-amber-500/20 text-amber-300 border-amber-500/30', icon: Calendar },
  registration_open: { label: 'Anmälan öppen', color: 'bg-yellow-500/20 text-yellow-300 border-yellow-500/30', icon: CheckCircle },
  ...
  return (
    <div className="min-h-screen bg-[#0F1513] pb-24 lg:pb-8">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-6">
        ...
        {/* Header Card with Yellow Theme */}
        <motion.div
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.4 }}
        >
          <Card className="bg-gradient-to-br from-[#121715] to-[#0F2917]/20 border border-[#F4743B]/30 shadow-[0_6px_18px_rgba(0,0,0,0.22)] rounded-[20px] overflow-hidden">
            {/* Banner with Yellow Gradient */}
            {cup.logo_url ? (
              <div className="h-48 bg-gradient-to-br from-[#F4743B]/20 to-[#E5683A]/10 relative">
                <img 
                  src={cup.logo_url} 
                  alt={cup.name}
            ...
            <CardContent className="p-6 -mt-12 relative z-10">
              ...
              <div className="flex items-start justify-between mb-4">
                <Badge className={`${statusConfig.color} px-4 py-2 text-sm font-bold border`}>
              ...
              {/* Signup Status */}
              {userParticipant && (
                <div className="mb-4 p-4 bg-[#F4743B]/10 border border-[#F4743B]/30 rounded-xl">
                  <div className="flex items-center gap-3">
                    <CheckCircle className="w-5 h-5 text-[#F4743B]" />
                    <div>
        ...
        {/* Tabs with Yellow Theme */}
        <Tabs value={activeTab} onValueChange={setActiveTab}>
          <TabsList className="grid grid-cols-4 lg:grid-cols-5 gap-2 bg-[#121715] border border-[#223029] p-2 rounded-xl">
            <TabsTrigger value="info" className="gap-2 data-[state=active]:bg-[#F4743B]/20 data-[state=active]:text-[#F4743B]">
              <Info className="w-4 h-4" />
              <span className="hidden sm:inline">Info</span>
            </TabsTrigger>
            
            <TabsTrigger value="signup" className="gap-2 data-[state=active]:bg-[#F4743B]/20 data-[state=active]:text-[#F4743B]">
              <UserPlus className="w-4 h-4" />
              <span className="hidden sm:inline">Anmälan</span>
            </TabsTrigger>
            
            <TabsTrigger value="schedule" className="gap-2 data-[state=active]:bg-[#F4743B]/20 data-[state=active]:text-[#F4743B]">
              <Layout className="w-4 h-4" />
              <span className="hidden sm:inline">Schema</span>
            </TabsTrigger>
            
            <TabsTrigger value="matches" className="gap-2 data-[state=active]:bg-[#F4743B]/20 data-[state=active]:text-[#F4743B]">
              <ListChecks className="w-4 h-4" />
              <span className="hidden sm:inline">Matcher</span>
            </TabsTrigger>

            {canManage && (
              <TabsTrigger value="admin" className="gap-2 data-[state=active]:bg-[#F4743B]/20 data-[state=active]:text-[#F4743B] border-[#F4743B]/30">
                <Shield className="w-4 h-4" />
                ...
                <Card className="bg-[#121715] border border-[#223029] rounded-[20px] p-6">
                  ...
                  {cup.rules && (
                    <div className="mb-6">
                      ...
                      <div className="max-h-96 overflow-y-auto p-4 bg-[#0F1513] rounded-xl border border-[#F4743B]/20">
                  ...
                  {cup.prize && (
                    <div className="mb-6">
                      ...
                      <div className="p-4 bg-gradient-to-r from-[#FFD700]/10 to-[#FFA500]/10 rounded-xl border border-[#FFD700]/30">
                        <p className="text-[#F4F7F5] font-semibold">{cup.prize}</p>
                      </div>
                    </div>
                  )}
                  ...
                  <div className="grid sm:grid-cols-2 gap-4">
                    <div className="p-4 bg-[#0F1513] rounded-xl border border-[#F4743B]/10">
                    ...
                    {cup.entry_fee > 0 && (
                      <div className="p-4 bg-[#0F1513] rounded-xl border border-[#F4743B]/10">
                        ...
                        <p className="text-lg font-bold text-[#F4743B]">{cup.entry_fee} kr</p>
                      </div>
                    )}
