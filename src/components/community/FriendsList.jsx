...
export default function FriendsList({ friends, incomingRequests = [], onAcceptRequest, onDeclineRequest }) {
  ...
  if (uniqueFriends.length === 0 && !hasIncomingRequests) {
    return (
    ...
    <div className="space-y-8">
      {hasIncomingRequests && (
        <div className="space-y-4">
          <h3 className="text-[#F4F7F5] font-semibold text-lg flex items-center gap-2">
            <UserPlus className="w-5 h-5 text-[#F4743B]" />
            Vänförfrågningar
            <Badge className="bg-[#F4743B] text-white border-0">{incomingRequests.length}</Badge>
          </h3>
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
            {incomingRequests.map((request) => (
              <Card key={request.id} className="bg-[#18221E] border border-[#F4743B]/30 shadow-lg rounded-[16px]">
                <CardContent className="p-5">
                  <div className="flex items-center justify-between mb-4">
                    <span className="text-[#F4F7F5] font-medium">Ny förfrågan</span>
                    <span className="text-xs text-[#B6C2BC]">Väntar svar</span>
                  </div>
                  
                  <div className="flex gap-3">
                    <Button 
                      onClick={() => onAcceptRequest(request.id)}
                      className="flex-1 bg-[#2BA84A] hover:bg-[#248232] text-white rounded-xl"
                    >
                      Acceptera
                    </Button>
                    <Button 
                      ...
                      variant="outline"
                      className="flex-1 border-[#223029] hover:bg-[#223029] text-[#B6C2BC] rounded-xl"
                    >
                      Neka
                    </Button>
                  </div>
