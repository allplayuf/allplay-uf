      ...
      <div className="relative z-10 p-6 sm:p-8 lg:p-10">
        ...
        >
          ...
          <StatCard 
            ...
            label="MVPs" 
            iconColor="text-[#F4743B]"
            borderColor="hover:border-[#F4743B]/50"
            gradient="bg-gradient-to-br from-[#F4743B]/30 to-[#F4743B]/10"
          />
          <StatCard 
            icon={Flame} 
            secondaryIcon={Zap} 
...
function StatCard({ 
  ...
  return (
    <motion.div whileHover={{ scale: 1.05, y: -5 }} className="relative group">
      <div className={`absolute inset-0 ${gradient} rounded-2xl blur-lg opacity-0 group-hover:opacity-100 transition-opacity`}></div>
      <div className={`relative bg-white/15 backdrop-blur-md rounded-2xl p-4 sm:p-5 border border-white/30 shadow-xl ${borderColor} transition-all`}>
        <div className="flex items-center justify-between mb-2">
          <Icon className={`w-5 h-5 sm:w-6 sm:h-6 ${iconColor.includes('/') ? iconColor : iconColor.replace('text-', 'text-')}`} strokeWidth={2.5} />
          <SecondaryIcon className={`w-3 h-3 sm:w-4 sm:h-4 ${iconColor === "text-white/80" ? "text-white/60" : iconColor + "/80"}`} />
        </div>
